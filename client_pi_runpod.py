# -*- coding: utf-8 -*-
"""
Complimentary Machine - Pi 5 Final Art Version (RunPod Serverless)
修改：
1. API 调用改为 RunPod Serverless 格式
2. 图片转 base64 发送
3. 添加 RunPod API Key 配置
"""
...
RUNPOD_API_KEY = os.environ.get("RUNPOD_API_KEY", "rpa_H5ETRU15A50X6VA2HWNPC25QPDP2JPOTB1I2BJXN1sevva")
RUNPOD_ENDPOINT_ID = os.environ.get("RUNPOD_ENDPOINT_ID", "6cvgjfk9wm8cfn")
...
def try_open_camera(max_retries=30, retry_delay=2.0):
    ...
    Args:
        max_retries: 最大重试次数（默认30次 = 最多等60秒）
        retry_delay: 每次重试间隔（秒）
    
    Returns:
        cv2.VideoCapture 对象，如果失败返回 None
    """
...
def camera_thread_func():
    ...
    while not stop_event.is_set():
        ...
        if ret:
            ...
            # 如果连续失败太多次，尝试重新连接摄像头
            if consecutive_failures > 30:  # 约3秒无响应
...
def tts_thread_func():
    while not stop_event.is_set():
        try:
            ...
            try:
                # stderr=None 让 piper 的加载信息输出，产生 glitch 效果
                ...
                # 关键：关闭父进程中的 piper.stdout 引用，这样 aplay 才能在 piper 结束时收到 EOF
...
# --- 4. API & Logic (RunPod Serverless) ---
def call_runpod_api(image, user_text=None):
    """
    调用 RunPod Serverless API
    图片转为 base64 发送
    """
    try:
        ...
        if user_text:
            payload["input"]["user_text"] = user_text
        
        # 发送请求 (runsync 同步模式)
        resp = requests.post(RUNPOD_API_URL, json=payload, headers=headers, timeout=60)
        
        if resp.status_code == 200:
            result = resp.json()
            # RunPod 返回格式: {"output": {"compliment": "..."}, "status": "COMPLETED"}
            if result.get("status") == "COMPLETED":
                output = result.get("output", {})
                if "error" in output:
                    return f"API Error: {output['error']}"
                return output.get("compliment", "No response")
            elif result.get("status") == "FAILED":
                return f"Job Failed: {result.get('error', 'Unknown')}"
            else:
...
def worker_func():
    ...
    while not stop_event.is_set():
        ...
        if img:
            resp = call_runpod_api(img, None)
            if "Error" not in resp and "Timeout" not in resp and "Failed" not in resp:
...
def main():
    # 检查配置
    if "你的" in RUNPOD_API_KEY or "你的" in RUNPOD_ENDPOINT_ID:
        print("⚠️  请先配置 RunPod API Key 和 Endpoint ID!")
        print("   方法1: 直接修改脚本顶部的配置")
        print("   方法2: 设置环境变量:")
        print("     export RUNPOD_API_KEY='your_api_key'")
        print("     export RUNPOD_ENDPOINT_ID='your_endpoint_id'")
        print()
        print(f"   当前 API URL: {RUNPOD_API_URL}")
        sys.exit(1)
    
    print(f"🚀 使用 RunPod Serverless API: {RUNPOD_API_URL}")
